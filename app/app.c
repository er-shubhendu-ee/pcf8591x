...
static int test_i2c(void) {
    ...
    driver_i2c_comm_port_t port = {.instanceConfig.pvCtx = NULL,
                                   .instanceConfig.id = 0,
                                   .portInitFn = mock_i2c_init,
                                   .portDeinitFn = mock_i2c_deinit,
                                   .portWriteMasterFn = mock_i2c_write,
                                   .portReadMasterFn = mock_i2c_read,
                                   .getTimestamp = mock_i2c_get_timestamp,
                                   .portIsTimeoutFn = mock_i2c_is_timeout};
    ...
    st = DRIVER_I2C_STATUS_OK;
    do {
        st = driver_i2c_transfer(&port, &writeTxn);
    } while (DRIVER_I2C_STATUS_BUSY == st);
    ...
    st = DRIVER_I2C_STATUS_OK;
    do {
        st = driver_i2c_transfer(&port, &txn);
    } while (DRIVER_I2C_STATUS_BUSY == st);
